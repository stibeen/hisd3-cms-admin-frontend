...
import { Button, Layout, Menu, theme, ConfigProvider, Breadcrumb, Modal, message } from 'antd';
...
const MainLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    const [logout, { loading }] = useMutation(LOG_OUT_MUTATION);
    ...
    const breadcrumbItems = [
        ...
        ...pathSnippets.map((snippet, index) => {
            ...
            const displayName = snippet.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
    ...
    return (
        ...
        >
            ...
            <Layout style={{ minHeight: '100vh' }}>
                <Sider trigger={null} collapsible collapsed={collapsed} style={{ position: 'sticky', top: 0, height: '100vh', backgroundColor: '#1280ED' }}>
                    <div className='p-2 flex bg-white m-1.5 rounded-lg items-center justify-center transition-all duration-300'>
                        <img src="/hisd3-logo.svg" alt="HISD3 Logo" className='h-8 w-auto' />
                        {!collapsed && (
                    ...
                    <Menu
                        ...
                        items={[
                            ...
                            { key: '2', icon: <FileTextOutlined />, label: <Link to="/all-posts">Posts</Link> },
                            { key: '3', icon: <QuestionCircleOutlined />, label: <Link to="/inquiries">Inquiries</Link> },
                            { key: '4', icon: <ShoppingOutlined />, label: <Link to="/products">Products</Link> },
                            { key: '5', icon: <TeamOutlined />, label: <Link to="/team">Team</Link> },
                            { key: '6', icon: <SettingOutlined />, label: <Link to="/settings">Settings</Link> },
                            { key: '7', icon: <LogoutOutlined />, label: 'Logout', onClick: showLogoutConfirm },
                        ]}
                    />
                </Sider>
                <Layout>
                    <Header className='flex items-center' style={{ position: 'sticky', top: 0, zIndex: 1000, padding: 0, background: colorBgContainer }}>
                        <Button
                            type="text"
                    ...
                    <Content style={{ margin: '16px', padding: 16, minHeight: 280, background: colorBgContainer, borderRadius: borderRadiusLG, border: '1px solid #1280ED' }}>
                        {children}
                    </Content>
                    <Footer className='text-center' style={{ padding: 20, background: colorBgContainer }}>
