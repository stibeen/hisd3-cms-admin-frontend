...
function ApolloConnectionTest() {
  ...
  if (error) return (
    <div className="p-4 bg-red-100 text-red-800 rounded-lg m-4 border-red-200 border">
      ...
      <p className="text-xs mt-2 text-red-600 font-mono">Endpoint: {import.meta.env.VITE_GRAPHQL_API_URL}</p>
    </div>
  );
  return (
    <div className="p-4 bg-green-50 text-green-800 rounded-lg m-4 border-green-200 border shadow-sm">
      ...
      <p className="text-sm">Loaded {data?.__schema?.types?.length} types from the backend schema.</p>
    </div>
  );
}
...
function DashboardApp() {
  ...
  const postStat = [
    ...
    },
    {
      ...
      length: data?.adminProducts?.filter((item) => item.isActive === true).length ?? 0,
      icon: <Package className="w-6 h-6 text-emerald-600" />,
      bgClass: "bg-emerald-50"
    },
    {
      title: "New Inquiries",
      length: data?.inquiries.filter((item) => item.status === "UNREAD").length ?? 0,
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link to="/all-posts/create-new-post">
            <Button type='primary' icon={<PlusCircle />} className='p-5!'>New Post</Button>
          </Link>
          <Link to="/products/create-new-product">
            <Button type='primary' icon={<Package />} className='p-5!'>New Product</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <Link to="/all-posts" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1 transition-colors">
          ...
          <div className="p-0 flex-1">
            ...
            {error && <div className="p-4 text-center text-red-500">Error loading articles: {error.message}</div>}
            <ul className="divide-y divide-gray-100">
              {(data?.adminArticles?.length ?? 0) > 0 ? (
                displayedArticles?.map((article) => (
                  <li key={article.id} className="p-6 flex items-center justify-between hover:bg-gray-50 transition-colors group">
                    <div className="flex gap-4">
                      ...
                      <div>
                        <h3 className="text-md font-medium text-gray-900 group-hover:text-blue-600 transition-colors cursor-pointer capitalize">{article.title}</h3>
                        <div className="text-sm text-gray-500 flex items-center gap-2">
                          <span className='capitalize'>{article?.author?.username}</span>
                          <span>•</span>
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDistanceToNow(new Date(article.updatedAt), { addSuffix: true })}</span>
                        </div>
                      </div>
                    </div>
                    <Tag
                      ...
                      variant='solid'
                      icon={article.status === `PUBLISHED` ? <CheckCircleFilled /> : <ContainerOutlined />}
                      style={{
                        padding: '4px 16px',
                        borderRadius: '20px',
                        backgroundColor: article.status === `PUBLISHED` ? '#E8F8F3' : '#F1F5F9', // Subtle background
                        color: article.status === `PUBLISHED` ? '#10B981' : '#64748B', // Tailwind emerald-500 or slate-500
                        border: 'none',
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <Link to="/inquiries" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1 transition-colors">
              View all <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          {/* Inquiries List */}
              ...
              {(filteredInquiries.length ?? 0) > 0 ? (
                displayedInquiries?.map((inquiry) => (
                  <li key={inquiry.id} className="p-5 flex items-start gap-4 hover:bg-gray-50 transition-colors cursor-pointer">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-gray-900">{inquiry.name}</h4>
                      <p className="text-sm text-gray-500 line-clamp-1 mt-0.5">{formatDistanceToNow(new Date(inquiry.createdAt), { addSuffix: true })}</p>
                    </div>
                  </li>
                ))) : (
                <Empty className='py-10' />
  ...
  return (
    <Card className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex justify-between items-start">
        <div>
          <Title level={5} className="text-sm! font-medium! text-gray-500!">{label}</Title>
          <Title level={3} className="text-3xl! font-bold! text-[#1280ED]! mt-2!">{value}</Title>
        </div>
