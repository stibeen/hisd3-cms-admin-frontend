import { CheckCircleFilled, ContainerOutlined, InboxOutlined, EditOutlined, PlusOutlined, UndoOutlined, DeleteOutlined } from '@ant-design/icons';
import { createFileRoute, Link, useNavigate } from '@tanstack/react-router'
import { useState, useMemo } from 'react';
import { Typography, Button, Table, Input, Tag, Tooltip, Divider, Select, Empty, Modal } from 'antd';
import type { TableProps } from 'antd';
import { POSTS_PAGE_QUERY } from '@/graphql/queries';
import { formatDistanceToNow } from 'date-fns';
import { useReadQuery, useMutation } from "@apollo/client/react";
import { UPDATE_ARTICLE_BY_ID_MUTATION, RESTORE_ARTICLE_BY_ID_MUTATION, HARD_DELETE_ARTICLE_BY_ID_MUTATION } from '@/graphql/mutations';
...
function RouteComponent() {
  ...
  const [restoreArticle, { loading: restoreArticleLoading }] = useMutation(RESTORE_ARTICLE_BY_ID_MUTATION);
  const [hardDeleteArticle, { loading: hardDeleteArticleLoading }] = useMutation(HARD_DELETE_ARTICLE_BY_ID_MUTATION);
  const [messageApi, messageContextHolder] = message.useMessage();
  const [modal, contextHolder] = Modal.useModal();

  const showConfirm = (id: string, type: 'archive' | 'unarchive' | 'hard-delete') => {
    modal.confirm({
      title: type === 'archive' ? 'Archive?' : type === 'unarchive' ? 'Unarchive?' : 'Hard Delete?',
      content: type === 'archive' ? 'Are you sure you want to archive this article?' : type === 'unarchive' ? 'Are you sure you want to unarchive this article?' : 'Are you sure you want to hard delete this article?',
      onOk() {
        type === 'archive' ? handleArchiveButton(id) : type === 'unarchive' ? handleUnarchiveButton(id) : handleHardDeleteButton(id);
      },
    });
  }
  ...
  const columns: TableProps['columns'] = useMemo(() => [
    ...
    },
    {
      ...
      dataIndex: 'updatedAt',
      key: 'updatedAt',
      render: (text: string) => <span>{formatDistanceToNow(new Date(text), { addSuffix: true })}</span>
    },
    {
      title: 'Status',
      dataIndex: 'status',
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className='flex gap-2 justify-center'>
          {record.status !== 'ARCHIVED' && (record.status === 'PUBLISHED' || record.status === 'DRAFT') &&
            <Tooltip title="Edit">
          ...
          {(record.status === 'PUBLISHED' || record.status === 'DRAFT') && (
            <Tooltip title="Archive">
              <Button type="text" danger icon={<InboxOutlined />} onClick={() => showConfirm(record.id, 'archive')} disabled={updateArticleLoading} />
            </Tooltip>
          )}
          {record.status === 'ARCHIVED' && (
            <>
              <Tooltip title="Unarchive">
                <Button type="text" style={{ color: '#10B981' }} icon={<UndoOutlined />} onClick={() => showConfirm(record.id, 'unarchive')} disabled={restoreArticleLoading} />
              </Tooltip>
              <Tooltip title="Hard Delete">
                <Button type="text" danger icon={<DeleteOutlined />} onClick={() => showConfirm(record.id, 'hard-delete')} disabled={hardDeleteArticleLoading} />
              </Tooltip></>
          )}
        </div>
      )
    ...
    <div className="p-6">
      ...
      <div className='flex justify-between items-end mb-6'>
        <div>
          ...
          <p className='text-gray-500'>Manage and organize your content across all categories.</p>
        </div>
            ...
            <div className='flex gap-4'>
              ...
              <Select
                ...
                options={[
                  ...
                  ...(articleData?.categories?.map((c: any) => ({ value: c.name, label: c.name })) || [])
                ]}
              />
            </div>
          )}
          pagination={articleData?.adminArticles?.length > 5 ? { pageSize: 5, showTotal: (total: number) => `${total} articles` } : false}
        />
      ) : (
